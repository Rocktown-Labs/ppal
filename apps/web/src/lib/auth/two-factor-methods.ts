...
export const isTwoFactorRedirect = (data: unknown): data is TwoFactorRedirect =>
  typeof data === "object" &&
  data !== null &&
...
export const parseTwoFactorMethods = (methods?: unknown): TwoFactorMethod[] => {
...
export const storeTwoFactorMethods = (methods?: unknown): void => {
...
export const readTwoFactorMethods = (): TwoFactorMethod[] => {
  if (typeof sessionStorage === "undefined") {
  ...
  try {
    ...
    return methods.length > 0 ? methods : TWO_FACTOR_METHODS;
  } catch {
    return TWO_FACTOR_METHODS;
  }
};
...
export const clearTwoFactorMethods = (): void => {
  if (typeof sessionStorage === "undefined") {
    return;
  }
